...
export async function approveMembershipRequest(db, { uid, studentId, expectedEmail, actorUid }) {
    ...
    return runTransaction(db, async transaction => {
        ...
        if (existing.exists() || member.data().Google_UID) throw new Error('帳號或成員已有綁定，請先由管理員解除原授權。');
        ...
        if (member.data().Status !== 'Active') throw new Error('僅能開通在職／在學成員。');
        // First approval always creates an ordinary member. Admin promotion is separate.
        const after = { ...member.data(), Student_ID: studentId, Role: 'User',
    ...
    return runTransaction(db, async transaction => {
        ...
        if (before.Google_UID) {
            ...
            if (!entry.exists() || entry.data().student_id !== studentId) {
                throw new Error('缺少已核准的帳號對應，請先完成遷移核對。');
            }
        }
...
export async function revokeApprovedMembership(db, studentId, actorUid, { deleteMember = false } = {}) {
    ...
    return runTransaction(db, async transaction => {
        ...
        if (before.Google_UID) transaction.delete(doc(db, 'member_access', before.Google_UID));
        if (deleteMember) {
