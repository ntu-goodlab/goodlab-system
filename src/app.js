...
const app = {
    ...
    data: {
        ...
        routines: [], bulletins: [],
        projects: [], employments: [], access_requests: [], duty_assignments: [], duty_people: [], duty_events: [], duty_rotation: []
    },
    ...
    helpDocs: {
        ...
        'duty': `
            ...
            <ul style="margin-top: 10px; padding-left: 20px; line-height: 1.6;">
                ...
                <li><strong>自動輪值：</strong>完成後輪到下一位；未完成由原輪值者自動順延，不需要每週核准。代做只限邀請的該週。</li>
        ...
        'duty-history': `
            <h3 style="color: var(--primary); border-bottom: 2px solid var(--border-color); padding-bottom: 8px; margin-bottom: 12px;">值日生執行紀錄</h3>
            <p>依週次查看已完成與未完成順延的值日紀錄。展開後只顯示叫貨內容與本週留言，不顯示一般清潔細項。</p>`,
        'routine': `
    ...
    userHelpDocs: {
        ...
        'duty': `
            ...
            <ul><li>完成清潔，並確認每項耗材為「足夠」或「已叫貨」後才能提交；「待叫貨」可暫存，但不能提交。</li><li>本週負責人可請人代做；指定本人接受後才交接，不交換未來輪值。</li><li>完成後自動輪下一位；未完成由原輪值者自動順延，不需要每週核准。代做不自動延續。</li><li>留言可填寫異常或補充內容；邀請的私人交接事項僅雙方與管理員可見。</li></ul>`,
        'duty-history': `
    ...
    switchTab: function(tabId, fromRoute = false) {
        ...
        // 桌面版側邊欄 highlight
        const parentTabId = tabId === 'duty-history' ? 'duty' : tabId;
        document.querySelectorAll('.nav-item').forEach(btn => {
            const isMatch = btn.getAttribute('onclick') && btn.getAttribute('onclick').includes("'" + parentTabId + "'");
    ...
    getRealtimeConfig: function() {
        return {
            ...
            duty_rotation: { dataKey: 'duty_rotation', withId: true, onData: () => { this.renderDuty(); this.renderOverview(); } },
            ...
            duty_records: { dataKey: 'duty_records', withId: true, onData: () => { this.renderDuty(); this.renderDutyHistory(); this.renderOverview(); } },
    ...
    syncRealtimeListeners: function(profile) {
        ...
        if (this.approvedAccessEnabled) {
            ...
            allowedByProfile.User.push('duty_rotation');
            allowedByProfile.Admin.push('access_requests', 'duty_assignments', 'duty_directory', 'duty_events', 'duty_rotation');
        }
        ...
        allowed.forEach(name => {
            ...
            const unsubscribe = onSnapshot(source, snapshot => {
                ...
                item.onData();
                if (['members', 'member_directory'].includes(name)) this.syncAutomaticDuty?.();
